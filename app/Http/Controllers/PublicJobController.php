...
class PublicJobController extends Controller
{
    public function index(Request $r)
    {
        ...
        $jobs = JobListing::query()->published()->with(['company:id,name,logo_path', 'category:id,name'])
            ...
            ->when($r->string('posted')->toString(), function ($q, $v) {
                $days = ['day' => 1, 'week' => 7, 'month' => 30][$v] ?? null;
                if ($days) {
        ...
        return Inertia::render('Jobs', ['canLogin' => Route::has('login'), 'canRegister' => Route::has('register'), 'databaseJobs' => $jobs,
