...
class DatabaseSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        foreach ([['Technology', 'technology'], ['Commerce', 'commerce'], ['Hotels & Tourism', 'hotels-tourism'], ['Education', 'education'], ['Financial Services', 'financial-services'], ['Construction', 'construction']] as [$name,$slug]) {
            JobCategory::firstOrCreate(['slug' => $slug], ['name' => $name]);
        }
        foreach ([['Free', 0, 1], ['Growth', 4900, 10], ['Business', 12900, 50]] as [$name,$price,$credits]) {
            Plan::firstOrCreate(['name' => $name], ['price_cents' => $price, 'job_credits' => $credits, 'features' => ['Dummy checkout', 'Job credits', 'Employer analytics']]);
        }
        foreach ([['Admin User', 'admin@example.com', 'admin'], ['Employer User', 'employer@example.com', 'employer'], ['Candidate User', 'candidate@example.com', 'candidate']] as [$name,$email,$role]) {
            User::firstOrCreate(['email' => $email], ['name' => $name, 'role' => $role, 'email_verified_at' => now(), 'password' => Hash::make('password')]);
        }
